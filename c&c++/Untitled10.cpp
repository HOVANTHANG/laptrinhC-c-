...
//Vi so cot va so hang cua ma tran A, B bang nhau nen ta khong c?n truy?n vào bi?n n1
     ...
     for(i=0;i<n;i++) 
        for(j=0;j<m1;j++) {
         int tt=0;
           for(k=0;k<m;k++)
               {
                  tt = tt + (a[i][k] * b[k][j]); //Công th?c tính C[i][j]
                }
            c[i][j] = tt;
    }
     
    printf("\nTICH 2 MA TRAN A B\n");
    output(n,m1,c); //Ma tr?n tích có kích thý?c b?ng s? hàng c?a A và s? c?t cua B
}
